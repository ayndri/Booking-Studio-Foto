...
class ContentController extends Controller
{
    ...
    private function applySectionFilter(Builder $query, string $section, ?string $homeMenu = null): void
    {
        ...
        if ($section === self::SECTION_ABOUT) {
            $query->where(function (Builder $subQuery) {
                $subQuery->where('key', 'about_page')
                         ->orWhere('key', 'like', 'about_value_%')
                         ->orWhere('key', 'like', 'about_story_%');
            });
            return;
        }
        ...
        if ($section === self::SECTION_TERMS) {
            $query->where(function (Builder $subQuery) {
                $subQuery->where('key', 'terms_page')
                         ->orWhere('key', 'like', 'terms_item_%')
                         ->orWhere('key', 'like', 'terms_flow_%')
                         ->orWhere('key', 'like', 'terms_extra_%');
            });
            return;
        }
    ...
    private function contentTypeOptions(string $section, ?string $homeMenu = null): array
    {
        ...
        if ($section === self::SECTION_ABOUT) {
            return [
                ['value' => 'about_page',       'label' => 'Intro Halaman Tentang Kami', 'mode' => 'single', 'key' => 'about_page',    'requires_image' => false],
                ['value' => 'about_value_new',  'label' => 'Core Value Baru',            'mode' => 'repeat', 'prefix' => 'about_value_', 'requires_image' => false],
                ['value' => 'about_story_new',  'label' => 'Cerita / Story Baru',        'mode' => 'repeat', 'prefix' => 'about_story_', 'requires_image' => false],
            ];
        }
        ...
        if ($section === self::SECTION_TERMS) {
            return [
                ['value' => 'terms_page',       'label' => 'Intro Syarat & Ketentuan',    'mode' => 'single', 'key' => 'terms_page',    'requires_image' => false],
                ['value' => 'terms_item_new',   'label' => 'Poin Utama S&K Baru',         'mode' => 'repeat', 'prefix' => 'terms_item_',  'requires_image' => false],
                ['value' => 'terms_flow_new',   'label' => 'Langkah Alur Booking Baru',   'mode' => 'repeat', 'prefix' => 'terms_flow_',  'requires_image' => false],
                ['value' => 'terms_extra_new',  'label' => 'Ketentuan Tambahan Baru',     'mode' => 'repeat', 'prefix' => 'terms_extra_', 'requires_image' => false],
            ];
        }
