...
class HomeController extends Controller
{
    ...
    public function about(): View
    {
        return view('frontend.about', [
            'about' => $this->getContent(
                'about_page',
                'Tentang Kami',
                'UPFotoStudio hadir untuk membantu kebutuhan foto personal, keluarga, hingga branding bisnis.'
            ),
            'aboutValues' => $this->getStructuredListByPrefix('about_value_', [
                ['title' => 'Kolaboratif',         'description' => 'Kami terbiasa bekerja bareng make-up artist, stylist, dan tim konten klien.'],
                ['title' => 'Transparan',           'description' => 'Informasi harga, durasi, dan output dijelaskan dari awal supaya ekspektasi lebih jelas.'],
                ['title' => 'Ramah & Nyaman',       'description' => 'Studio bersih, staf ramah, dan suasana sesi foto dibuat senyaman mungkin.'],
                ['title' => 'Cepat Beradaptasi',    'description' => 'Kami fleksibel menyesuaikan konsep visual sesuai kebutuhan personal maupun bisnis.'],
            ]),
            'aboutStories' => $this->getSimpleListByPrefix('about_story_', [
    ...
    public function terms(): View
    {
        return view('frontend.terms', [
            ...
            'termFlow' => $this->getStructuredListByPrefix('terms_flow_', [
                ['step' => '01', 'title' => 'Pilih Paket & Slot',      'description' => 'Pilih studio, tanggal, dan jam yang masih tersedia.'],
                ['step' => '02', 'title' => 'Isi Data Pemesan',         'description' => 'Data kontak aktif dibutuhkan agar status booking mudah dipantau.'],
                ['step' => '03', 'title' => 'Konfirmasi Pembayaran',    'description' => 'Sistem membuat invoice dan menunggu status pembayaran QRIS berhasil.'],
                ['step' => '04', 'title' => 'Booking Dikonfirmasi',     'description' => 'Setelah transaksi sukses, status booking berubah menjadi CONFIRMED otomatis.'],
            ], true),
            'termExtraItems' => $this->getSimpleListByPrefix('terms_extra_', [
    ...
    /**
     * Ambil daftar konten berbasis prefix key, kembalikan array ['title','description'].
     * Jika $withStep=true, tambahkan field 'step' (nomor urut dua digit).
     */
    private function getStructuredListByPrefix(string $prefix, array $fallback, bool $withStep = false): array
    {
        $items = WebsiteContent::query()
            ...
            ->values()
            ->map(function ($item, int $index) use ($withStep): array {
                $data = ['title' => $item->title, 'description' => (string) ($item->content ?? '')];
                if ($withStep) {
                    $data['step'] = str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT);
                }
                return $data;
            })
            ->all();
